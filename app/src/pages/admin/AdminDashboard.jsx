      ...
      <header className="bg-[#2C3D30] text-[#FAF8F5]">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 h-18 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/logo-FamilyHealth.webp" alt="Logo" className="h-8 w-auto" />
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setShowUserModal(true)} className="flex items-center gap-1.5 text-sm bg-white/10 hover:bg-white/20 rounded-full px-4 py-2 transition-colors">
              ...
              {editing.image_url && (
                <div className="mt-3 p-2 bg-[#E7E0D3] rounded-xl overflow-hidden flex justify-center">
                  {editing.image_url.toLowerCase().endsWith(".mp4") || editing.image_url.toLowerCase().endsWith(".mov") ? (
                    <video src={editing.image_url} controls className="max-h-48 w-auto rounded-lg" />
                  ) : (
                    <img src={editing.image_url} alt="Preview" className="max-h-48 w-auto rounded-lg" />
                  )}
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-[#FAF8F5] rounded-3xl w-full max-w-sm p-7">
            ...
            <div className="relative mb-4">
              ...
              <button type="button" onClick={() => setShowNewUserPass(!showNewUserPass)} className="absolute right-3 top-2.5 text-[#5C6656] hover:text-[#2C3D30]">
                {showNewUserPass ? <EyeOff size={18} /> : <Eye size={18} />}
              </button>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowUserModal(false)} className="flex-1 py-2 rounded-full border">{t("admin_cancel")}</button>
              <button onClick={createUser} className="flex-1 py-2 rounded-full bg-[#4A5D4E] text-white">{t("admin_save")}</button>
            </div>
          </div>
        </div>
      )}
